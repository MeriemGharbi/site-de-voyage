    ...
    <select onchange="window.location.href=this.value" class="form-select">
        ...
        if (count($categories) > 0) {
            foreach ($categories as $row) {
                echo "<option value='activitiesjoinfront.php?category_id=" . $row['id_category'] . "'>" . $row['nom_category'] . "</option>";
            }
        } else {
...
<script>
    ...
    $("#live_search").keyup(function(){
        ...
        if(input != ""){
            $.ajax({
                url: "../../controller/activite/livesearchfront.php",
                method: "POST",
                data: {input: input},
...
?>
        <div class="activity">
            <div class="activity-details">
                ...
                <p class="activity-average-rating"><?= $activity['average_rating'] != 0.0 ? number_format($activity['average_rating'], 1) : 'No ratings yet' ?>                <!DOCTYPE html>
<html>
<head>
